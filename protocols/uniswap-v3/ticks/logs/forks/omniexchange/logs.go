...
import (
	"github.com/defistate/defistate/protocols/uniswap-v3/abi/forks/omniexchange" // Assumes this is where your ABI is located
...
var (
	...
	mintEvent = omniexchange.OmniExchangeABI.Events["Mint"]
	burnEvent = omniexchange.OmniExchangeABI.Events["Burn"]
)

// OmniExchangePoolsUpdatedInBlock parses a slice of logs and returns the pool addresses
// and a corresponding list of unique tick indexes that were updated via Mint or Burn events.
func OmniExchangePoolsUpdatedInBlock(logs []types.Log) (pools []common.Address, updatedTicks [][]int64, err error) {
