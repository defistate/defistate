...
func ExtractSwapsFromLogs(logs []types.Log) (
	...
	swapEvent, ok := abi.PancakeswapV3ABI.Events["Swap"]
	if !ok {
	...
	for _, vLog := range logs {
		...
		if len(vLog.Topics) > 0 && vLog.Topics[0] == swapEventID {
			...
			unpacked, err := abi.PancakeswapV3ABI.Unpack("Swap", vLog.Data)
			if err != nil {
...
func ExtractMintsAndBurnsFromLogs(logs []types.Log) (pools []common.Address) {
	...
	mintEventID := abi.PancakeswapV3ABI.Events["Mint"].ID
	burnEventID := abi.PancakeswapV3ABI.Events["Burn"].ID
