...
func ExtractSwapsFromLogs(logs []types.Log) (
	...
	swapEvent, ok := abi.UniswapV3ABI.Events["Swap"]
	if !ok {
	...
	for _, vLog := range logs {
		...
		if len(vLog.Topics) > 0 && vLog.Topics[0] == swapEventID {
			...
			unpacked, err := abi.UniswapV3ABI.Unpack("Swap", vLog.Data)
			if err != nil {
...
func ExtractMintsAndBurnsFromLogs(logs []types.Log) (pools []common.Address) {
	...
	mintEventID := abi.UniswapV3ABI.Events["Mint"].ID
	burnEventID := abi.UniswapV3ABI.Events["Burn"].ID
