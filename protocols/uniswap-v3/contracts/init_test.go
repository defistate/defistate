...
func TestNewPoolInitializer(t *testing.T) {
	...
	t.Run("successfully fetches all pool info via multicall", func(t *testing.T) {
		...
		mockReturnValues := []interface{}{
			...
			[]interface{}{mockSqrtPriceX96, big.NewInt(mockTick), uint16(0), uint16(0), uint16(0), uint8(0), true},
			mockLiquidity,
		}
		...
		for i, name := range callNames {
			callData, err := uniswapv3abi.UniswapV3ABI.Pack(name)
			...
			if name == "slot0" {
				...
				returnData, err = uniswapv3abi.UniswapV3ABI.Methods[name].Outputs.Pack(slot0Values...)
			} else {
				returnData, err = uniswapv3abi.UniswapV3ABI.Methods[name].Outputs.Pack(mockReturnValues[i])
			}
			require.NoError(t, err)
	...
	t.Run("returns error when a subcall fails", func(t *testing.T) {
		...
		token0Return, err := uniswapv3abi.UniswapV3ABI.Methods["token0"].Outputs.Pack(common.Address{})
		require.NoError(t, err)
		token1Return, err := uniswapv3abi.UniswapV3ABI.Methods["token1"].Outputs.Pack(common.Address{})
		require.NoError(t, err)
