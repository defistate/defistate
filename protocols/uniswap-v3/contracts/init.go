...
// NewPoolInitializer creates a provider that fetches all core pool initrmation in a single, batched multicall.
...
func NewPoolInitializer(multicallAddress common.Address) func(
	...
	return func(
		...
		for i, name := range callNames {
			callData, packErr := uniswapv3abi.UniswapV3ABI.Pack(name)
			if packErr != nil {
		...
		err = uniswapv3abi.UniswapV3ABI.UnpackIntoInterface(&token0, "token0", multicallResults[0].ReturnData)
		if err != nil {
		...
		err = uniswapv3abi.UniswapV3ABI.UnpackIntoInterface(&token1, "token1", multicallResults[1].ReturnData)
		if err != nil {
		...
		var feeBig *big.Int
		err = uniswapv3abi.UniswapV3ABI.UnpackIntoInterface(&feeBig, "fee", multicallResults[2].ReturnData)
		if err != nil {
		...
		err = uniswapv3abi.UniswapV3ABI.UnpackIntoInterface(&tickSpacingBig, "tickSpacing", multicallResults[3].ReturnData)
		if err != nil {
		...
		err = uniswapv3abi.UniswapV3ABI.UnpackIntoInterface(&slot0Result, "slot0", multicallResults[4].ReturnData)
		if err != nil {
		...
		err = uniswapv3abi.UniswapV3ABI.UnpackIntoInterface(&liquidity, "liquidity", multicallResults[5].ReturnData)
		if err != nil {
