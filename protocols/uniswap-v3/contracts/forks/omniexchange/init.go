...
func NewPoolInitializer(multicallAddress common.Address) func(
	...
	return func(
		...
		for i, name := range callNames {
			callData, packErr := omniexchangeabi.OmniExchangeABI.Pack(name)
			if packErr != nil {
		...
		err = omniexchangeabi.OmniExchangeABI.UnpackIntoInterface(&token0, "token0", multicallResults[0].ReturnData)
		if err != nil {
		...
		err = omniexchangeabi.OmniExchangeABI.UnpackIntoInterface(&token1, "token1", multicallResults[1].ReturnData)
		if err != nil {
		...
		var feeBig *big.Int
		err = omniexchangeabi.OmniExchangeABI.UnpackIntoInterface(&feeBig, "fee", multicallResults[2].ReturnData)
		if err != nil {
		...
		err = omniexchangeabi.OmniExchangeABI.UnpackIntoInterface(&tickSpacingBig, "tickSpacing", multicallResults[3].ReturnData)
		if err != nil {
		...
		err = omniexchangeabi.OmniExchangeABI.UnpackIntoInterface(&slot0Result, "slot0", multicallResults[4].ReturnData)
		if err != nil {
		...
		err = omniexchangeabi.OmniExchangeABI.UnpackIntoInterface(&liquidity, "liquidity", multicallResults[5].ReturnData)
		if err != nil {
