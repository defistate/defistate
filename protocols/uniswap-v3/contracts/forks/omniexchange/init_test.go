...
func TestNewPoolInitializer(t *testing.T) {
	...
	t.Run("successfully fetches all pool info via multicall", func(t *testing.T) {
		...
		for i, name := range callNames {
			callData, err := omniexchangeabi.OmniExchangeABI.Pack(name)
			...
			if name == "slot0" {
				...
				returnData, err = omniexchangeabi.OmniExchangeABI.Methods[name].Outputs.Pack(slot0Values...)
			} else {
				returnData, err = omniexchangeabi.OmniExchangeABI.Methods[name].Outputs.Pack(mockReturnValues[i])
			}
			require.NoError(t, err)
	...
	t.Run("returns error when a subcall fails", func(t *testing.T) {
		...
		token0Return, err := omniexchangeabi.OmniExchangeABI.Methods["token0"].Outputs.Pack(common.Address{})
		require.NoError(t, err)
		token1Return, err := omniexchangeabi.OmniExchangeABI.Methods["token1"].Outputs.Pack(common.Address{})
		require.NoError(t, err)
