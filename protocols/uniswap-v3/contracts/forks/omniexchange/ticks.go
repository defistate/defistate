			...
			g.Go(func() error {
				...
				for j, result := range results {
					if result.Success && len(result.ReturnData) > 0 {
						err := omniexchangeabi.OmniExchangeABI.UnpackIntoInterface(&tickOutput, "ticks", result.ReturnData)
						if err != nil {
							return err
						}
...
func fetchTickInfoBatch(
	...
	for i, tick := range batch {
		callData, err := omniexchangeabi.OmniExchangeABI.Pack("ticks", big.NewInt(tick))
		if err != nil {
