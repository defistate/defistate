...
var (
	// Extracting method IDs from OmniExchange's specific ABI
	slot0Sig     = omniexchangeabi.OmniExchangeABI.Methods["slot0"].ID
	liquiditySig = omniexchangeabi.OmniExchangeABI.Methods["liquidity"].ID
	feeSig       = omniexchangeabi.OmniExchangeABI.Methods["fee"].ID
...
// NewBatchPoolInfoProvider fetches Tick, Liquidity, SqrtPriceX96, and Fee for OmniExchange pools.
func NewBatchPoolInfoProvider(
	...
	return func(ctx context.Context, poolAddrs []common.Address, getClient func() (ethclients.ETHClient, error), blockNumber *big.Int) (ticks []int64, liquidities []*big.Int, sqrtPriceX96s []*big.Int, fees []uint64, errs []error) {
		...
		// 4. Decode with OmniExchange-specific logic
		for i := 0; i < numPools; i++ {
			...
			if !resS0.Success || !resLiq.Success || !resFee.Success {
				errs[i] = fmt.Errorf("omniexchange pool fetch reverted")
				continue
			}

			// OmniExchange slot0() manual decode
			...
			// Liquidity (uint128) - occupies one 32-byte word
			if len(resLiq.ReturnData) >= 32 {
				liquidities[i] = new(big.Int).SetBytes(resLiq.ReturnData)
			}

			// Fee (uint24) - occupies one 32-byte word
