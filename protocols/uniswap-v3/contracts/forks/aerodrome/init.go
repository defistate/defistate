...
func NewPoolInitializer(multicallAddress common.Address) func(
	...
	return func(
		...
		for i, name := range callNames {
			callData, packErr := aerodromeabi.AerodromeABI.Pack(name)
			if packErr != nil {
		...
		err = aerodromeabi.AerodromeABI.UnpackIntoInterface(&token0, "token0", multicallResults[0].ReturnData)
		if err != nil {
		...
		err = aerodromeabi.AerodromeABI.UnpackIntoInterface(&token1, "token1", multicallResults[1].ReturnData)
		if err != nil {
		...
		var feeBig *big.Int
		err = aerodromeabi.AerodromeABI.UnpackIntoInterface(&feeBig, "fee", multicallResults[2].ReturnData)
		if err != nil {
		...
		err = aerodromeabi.AerodromeABI.UnpackIntoInterface(&tickSpacingBig, "tickSpacing", multicallResults[3].ReturnData)
		if err != nil {
		...
		err = aerodromeabi.AerodromeABI.UnpackIntoInterface(&slot0Result, "slot0", multicallResults[4].ReturnData)
		if err != nil {
		...
		err = aerodromeabi.AerodromeABI.UnpackIntoInterface(&liquidity, "liquidity", multicallResults[5].ReturnData)
		if err != nil {
