...
func TestNewPoolInitializer(t *testing.T) {
	...
	t.Run("successfully fetches all pool info via multicall", func(t *testing.T) {
		...
		mockReturnValues := []interface{}{
			...
			// The full 7-field struct for slot0
			[]interface{}{mockSqrtPriceX96, big.NewInt(mockTick), uint16(0), uint16(0), uint16(0), uint32(0), true},
			mockLiquidity,
		}
		...
		for i, name := range callNames {
			callData, err := pancakeswapv3abi.PancakeswapV3ABI.Pack(name)
			...
			if name == "slot0" {
				...
				returnData, err = pancakeswapv3abi.PancakeswapV3ABI.Methods[name].Outputs.Pack(slot0Values...)
			} else {
				returnData, err = pancakeswapv3abi.PancakeswapV3ABI.Methods[name].Outputs.Pack(mockReturnValues[i])
			}
			require.NoError(t, err)
	...
	t.Run("returns error when a subcall fails", func(t *testing.T) {
		...
		token0Return, err := pancakeswapv3abi.PancakeswapV3ABI.Methods["token0"].Outputs.Pack(common.Address{})
		require.NoError(t, err)
		token1Return, err := pancakeswapv3abi.PancakeswapV3ABI.Methods["token1"].Outputs.Pack(common.Address{})
		require.NoError(t, err)
