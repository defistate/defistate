...
func NewRouter(
	...
	// set allGetAmountOutFuncs for all uniswapv2 pools
	for _, pool := range state.UniswapV2.All() {
		...
		allGetAmountOutFuncs[poolIndex] = func(amountIn *big.Int, tokenInID, tokenOutID uint64) (*big.Int, error) {
			return uniswapv2math.GetAmountOut(amountIn, tokenInID, tokenOutID, p)
		}

		allGetReservesFuncs[poolIndex] = func(tokenInID, tokenOutID uint64) (*big.Int, *big.Int, error) {
			reserveTokenIn, reserveTokenOut, err := uniswapv2math.GetReserves(tokenInID, tokenOutID, p)
			if err != nil {
				return nil, nil, err
			}
	...
	// set allGetAmountOutFuncs for all uniswapv3 pools
	for _, pool := range append(state.UniswapV3.All(), state.AerodromeV3.All()...) {
		// get pool index from id
		poolIndex := poolToIndex[pool.IDs.Pool]
		// Check for Fee-On-Transfer tokens
		t0, ok0 := state.Tokens.GetByID(pool.IDs.Token0)
...
type PoolOverrides struct {
	UniswapV2   map[uint64]uniswapv2.Pool
	UniswapV3   map[uint64]uniswapv3.Pool
	AerodromeV3 map[uint64]uniswapv3.Pool
}
...
func (r *Router) FindBestSwapPathGreedy(
	...
	allUniswapOverridesV3 := map[uint64]uniswapv3.Pool{}
	if overrides.UniswapV3 != nil {
		maps.Copy(allUniswapOverridesV3, overrides.UniswapV3)
	}
	if overrides.AerodromeV3 != nil {
		maps.Copy(allUniswapOverridesV3, overrides.AerodromeV3)
	}

	// handle overrides by updating getAmountOutFuncs
	for _, pool := range allUniswapOverridesV3 {
	...
	// handle overrides by updating getAmountOutFuncs
	for _, pool := range overrides.UniswapV2 {
		// override getAmountOut function for that pool
		poolIndex, exists := r.poolToIndex[pool.IDs.Pool]
		if !exists {
			continue
...
func (r *Router) GetAmountOut(
	...
	for _, p := range path {
		...
		amountOut, err := getAmountOut(amountIn, p.TokenInID, p.TokenOutID)
		if err != nil {
			return nil, err
		}
