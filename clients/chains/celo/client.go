...
type State struct {
	...
	AerodromeV3      clients.IndexedUniswapV3
...
// State returns the current consistent DeFi snapshot (Pull pattern).
func (p *Client) State() *State {
	return p.latestState.Load()
}

// OnNewBlock registers a strategy function to execute upon block arrival (Push pattern).
func (p *Client) OnNewBlock(handler func(context.Context, *State) error) error {
	p.handlerMutex.Lock()
	defer p.handlerMutex.Unlock()

	if p.isRegistered {
		return errors.New("OnNewBlock handler is already registered for this client")
	}
...
type dataBuckets struct {
	...
	aerodromev3   []uniswapv3.PoolView
}
...
func (p *Client) processState(ctx context.Context, rawState *engine.State) (*State, error) {
	...
	var (
		...
		indexedAerodromeV3 clients.IndexedUniswapV3
	)
	...
	g.Go(func() error {
		var err error
		indexedAerodromeV3, err = p.indexers.UniswapV3.Index(AerodromeV3ProtocolID, buckets.aerodromev3, indexedTokenSystem, indexedPoolRegistry)
		return err
	})
	...
	state := &State{
		...
		AerodromeV3:      indexedAerodromeV3,
...
func (p *Client) extractBuckets(rawState *engine.State) (*dataBuckets, error) {
	...
	for protocolID, protocol := range rawState.Protocols {
		...
		case uniswapv2.UniswapV2ProtocolSchema:
			if protocolID == UniswapV2ProtocolID {
		...
		case uniswapv3.UniswapV3ProtocolSchema:
			...
			case AerodromeV3ProtocolID:
				buckets.aerodromev3 = protocol.Data.([]uniswapv3.PoolView)
			}
		}
	}
