...
import (
	...
	stateops "github.com/defistate/defistate/streams/jsonrpc/stateops/chains/arbitrum"
...
// Config defines the required dependencies for the Arbitrum client.
type Config struct {
...
func (p *Client) loop(ctx context.Context) {
	for {
		...
		case rawState, ok := <-p.stream.State():
			...
			handler := p.onNewBlock
			p.handlerMutex.RUnlock()

			if handler != nil {
				if err := handler(ctx, processed); err != nil {
					p.logger.Error("OnNewBlock handler error", "block", processed.Block.Number, "err", err)
				}
			}
