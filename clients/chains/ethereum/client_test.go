...
func TestClient_EthereumLifecycle(t *testing.T) {
	...
	t.Run("Pipeline Processing and Atomic State Retrieval", func(t *testing.T) {
		...
		// Inject multi-protocol raw state
		...
		require.Eventually(t, func() bool {
			mu.Lock()
			defer mu.Unlock()
			return handlerCalled && finalBlock == 42
		}, 2*time.Second, 10*time.Millisecond, "Ethereum OnNewBlock was not called")
	...
	t.Run("OnNewBlock Guard Enforcement", func(t *testing.T) {
		cfg, _, _ := setup()
	...
	t.Run("Context Shutdown Safety", func(t *testing.T) {
		cfg, mStream, _ := setup()
		ctx, cancel := context.WithCancel(context.Background())
