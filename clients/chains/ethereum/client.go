...
// Config defines the required dependencies for the Ethereum client.
type Config struct {
...
type Client struct {
	...
	// Pull Pattern: Atomic pointer for current state snapshots.
	latestState atomic.Pointer[State]

	// Push Pattern: Thread-safe callback registry.
...
func DialJSONRPCStream(
	...
	stateOps, err := ethstateops.NewStateOps(logger.With("component", "state-ops"), prometheusRegistry)
	if err != nil {
...
// State returns the current consistent DeFi snapshot.
...
// Option allows for fine-grained configuration.
