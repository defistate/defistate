...
import (
	...
	stateops "github.com/defistate/defistate/streams/jsonrpc/stateops/chains/pulsechain"
...
// State is the high-level, indexed and graphed representation of the DeFi state on PulseChain.
type State struct {
	Tokens           clients.IndexedTokenSystem
	Pools            clients.IndexedPoolRegistry
	UniswapV2        clients.IndexedUniswapV2
	PulseXV2         clients.IndexedUniswapV2
...
// Config defines the required dependencies for the PulseChain client.
type Config struct {
...
func DialJSONRPCStream(
	...
	if err != nil {
		return nil, fmt.Errorf("failed to dial pulsechain defistate stream: %w", err)
	}
...
func NewClient(ctx context.Context, cfg *Config) (*Client, error) {
	...
	m := &metrics{
		rawStateProcessed: factory.NewCounter(prometheus.CounterOpts{Name: "defistate_pulsechain_processed_total"}),
		processingDuration: factory.NewHistogramVec(prometheus.HistogramOpts{
			Name: "defistate_pulsechain_processing_duration_seconds", Buckets: prometheus.DefBuckets,
		}, []string{"stage"}),
	}
...
// State provides instantaneous, thread-safe access to the most recently indexed DeFi state.
...
// Returns an error if a handler is already registered for this client.
func (p *Client) OnNewBlock(handler func(context.Context, *State) error) error {
	p.handlerMutex.Lock()
	defer p.handlerMutex.Unlock()

	if p.isRegistered {
		return errors.New("OnNewBlock handler is already registered for PulseChain")
	}
...
func (p *Client) loop(ctx context.Context) {
	for {
		...
		case rawState, ok := <-p.stream.State():
			if !ok {
				p.logger.Error("Upstream PulseChain stream closed unexpectedly")
				return
			}

			processed, err := p.processState(ctx, rawState)
			if err != nil {
				p.logger.Error("PulseChain state processing failed", "block", rawState.Block.Number, "err", err)
				continue
			}
			...
			if currentHandler != nil {
				if err := currentHandler(ctx, processed); err != nil {
					p.logger.Error("PulseChain OnNewBlock callback error", "block", processed.Block.Number, "err", err)
				}
			}

		case err := <-p.stream.Err():
			p.logger.Error("PulseChain client fatal stream error", "err", err)
			return
		case <-ctx.Done():
			return
		}
...
func (p *Client) processState(ctx context.Context, rawState *engine.State) (*State, error) {
	...
	var (
		...
		indexedPulseXV2      clients.IndexedUniswapV2
	...
	g.Go(func() error {
		var err error
		indexedPulseXV2, err = p.indexers.UniswapV2.Index(PulseXV2ProtocolID, buckets.pulsexv2, indexedTokenSystem, indexedPoolRegistry)
		return err
	})
	g.Go(func() error {
		var err error
	...
	if err := g.Wait(); err != nil {
		return nil, fmt.Errorf("parallel indexing failed on PulseChain: %w", err)
	}
	...
	state := &State{
		...
		UniswapV2:        indexedUniswapV2,
		PulseXV2:         indexedPulseXV2,
		UniswapV3:        indexedUniswapV3,
		PancakeswapV3:    indexedPancakeswapV3,
...
func (p *Client) extractBuckets(rawState *engine.State) (*dataBuckets, error) {
	...
	for protocolID, protocol := range rawState.Protocols {
		...
		case uniswapv2.UniswapV2ProtocolSchema:
			...
			case PulseXV2ProtocolID:
				buckets.pulsexv2 = protocol.Data.([]uniswapv2.PoolView)
			}
